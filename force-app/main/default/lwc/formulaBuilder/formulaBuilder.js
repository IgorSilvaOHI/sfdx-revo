...
export default class FormulaBuilder extends LightningElement {
    ...
    @api get supportedSystemTypes() {
        return this.systemTypes.join(",");
    }

    set supportedSystemTypes(value) {
        this.systemTypes = this.splitValues(value);
            ...
            this.contextTypes.forEach((objType) => {
                let newContextFields = result.data[objType].map((curField) => {
                    return {
                        ...
                        value:
                            objType === this._objectName
                                ? curField.value
                                : "$" + objType + "." + curField.value
                    };
                });

                if (this.contextFields) {
                    this.contextFields =
                        this.contextFields.concat(newContextFields);
                } else {
    ...
    formulaChangedFlowEvent(event) {
        const valueChangeEvent = new FlowAttributeChangeEvent(
            "value",
            this.formula
        );
    ...
    insertInCurrentPosition(value, setCursor) {
        ...
        if (formulaTextArea) {
            if (value !== "") {
                let formulaStart =
                    this.formula.substring(0, formulaTextArea.selectionStart) +
                    " " +
                    value +
                    " ";
                let newFormulaString =
                    formulaStart +
                    this.formula.substring(formulaTextArea.selectionEnd);
    ...
    splitValues(originalString) {
        return originalString
            ? originalString.replace(/ /g, "").split(",")
            : [];
    }
}